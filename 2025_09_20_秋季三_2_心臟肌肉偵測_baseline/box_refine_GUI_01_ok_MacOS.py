...
#            print("請用左鍵刪除，右鍵功能暫不可用")
    ...
    def save_current_patient(self):
        ...
        # 合併所有病人（維持原排序）confidence 方便篩除score較低的樣本 -> filename 檔名排序方便觀察
        # 原DETR_1120_V2 top-1 WBF 是沒有任何排序的
        # 遞增排序 (Ascending) 預設是True，False遞減排序，不加ascendin預設就是遞增，由小到大。
        other_patients = self.df[self.df['patient_id'] != pid]
        final_df = pd.concat([other_patients, patient_data], ignore_index=True)
        final_df = final_df.sort_values('filename', ascending=True)
...
df_raw = load_yolo_submission("DETR_1120_V2_MainBody-p64-top1.txt")
