...
# Sending Zeta
# nv2111tf260Zeta
# echo "nv2111tf260Zeta";sh tf.ds.pipeline/run_taskCCS_PyArgv_zeta.sh
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "[Install env Zeta]"
fi
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "Zeta ${train_cmd}"
fi
...
then
    ...
    sh line_notify_start.sh "[DONE] Zeta ${train_cmd}"
fi
