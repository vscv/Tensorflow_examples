...
# Sending Kappa
# nv2111tf260kappa
# echo "nv2111tf260kappa";sh tf.ds.pipeline/run_taskCCS_PyArgv_kappa.sh
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "[Install env Kappa]"
fi
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "Kappa ${train_cmd}"
fi
...
then
    ...
    sh line_notify_start.sh "[DONE] Kappa ${train_cmd}"
fi
