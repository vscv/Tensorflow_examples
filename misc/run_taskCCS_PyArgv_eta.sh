...
# Sending Eta
# nv2111tf260Eta
# echo "nv2111tf260Eta";sh tf.ds.pipeline/run_taskCCS_PyArgv_eta.sh
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "[Install env Eta]"
fi
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "Eta ${train_cmd}"
fi
...
then
    ...
    sh line_notify_start.sh "[DONE] Eta ${train_cmd}"
fi
