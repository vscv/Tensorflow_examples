...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "[Install env aiotccsooa]"
fi
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "aiotccsooa ${train_cmd}"
fi
...
then
    ...
    sh line_notify_start.sh "[DONE] aiotccsooa ${train_cmd}"
fi
