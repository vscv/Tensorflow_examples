...
#
# combensembleacc
# echo "combensembleacc"; cd tf.ds.pipeline; sh Leaf_EnsembleModels_comb_acc.sh;
##################################################################################################
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "[Install env Allacc]"
fi
...
train_cmd="python3 2022_Leaf_rewrite-for-model-benchmark-forConvert2Py_EnsembleModels.py 30 31 imagenet1k crop plateau RA 512 512 1"


echo " * * Execute --> Allacc ${train_cmd} * *"
echo ""
echo ""
...
if [ ${message_on} = "True" ]
then
    sh line_notify_start.sh "Allacc ${train_cmd}"
fi
...
then
    ...
    sh line_notify_start.sh "[DONE] Allacc ${train_cmd}"
fi
