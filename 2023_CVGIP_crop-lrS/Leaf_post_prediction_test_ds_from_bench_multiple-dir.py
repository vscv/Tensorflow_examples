# 2022-03-09
#
# Predict the test ds accuracy from bench models and save to [model]_pred.npy for post ensemble.
#
#
# ==============================================================================
"""

$time python3 Leaf_post_prediction_test_ds_from_bench_multiple-dir.py
...
[each sub-job take about 2 hours.]
cd ~/tf.ds.pipeline;
sh install_env.sh 1>&- 2>&-;
time python3 Leaf_post_prediction_test_ds_from_bench_multiple-dir[plateau].py;

cd ~/tf.ds.pipeline;
sh install_env.sh 1>&- 2>&-;
time python3 Leaf_post_prediction_test_ds_from_bench_multiple-dir[CDR].py;

cd ~/tf.ds.pipeline;
sh install_env.sh 1>&- 2>&-;
time python3 Leaf_post_prediction_test_ds_from_bench_multiple-dir[WCD].py;
"""
...
#
# (1) predict in multiple directory from [logdir_name*]/model_name_test_*_N.vacc to get the [logdir_name*]/[model_name]/[N]/
# (2) get the model parameter number and the disk size
#

""" t_acc is a little different between model.evaluate and model..predict_on_batch

for example:
./imagenet_crop_512x512_plateau_AA_bs32/InceptionV3/InceptionV3_test_0.8725459575653076_2.vacc

model.evaluate --> Test dataset accuracy: 0.8725459575653076
model..predict_on_batch ---> accuracy  =  0.8725459644749143 ??
                                                   |

./imagenet_crop_512x512_WCD_AA_bs32/ResNet101/ResNet101_test_0.8675599694252014_0.vacc
model.evaluate --> Test dataset accuracy: 0.8675599694252014
model..predict_on_batch ---> accuracy  =  0.8675599875350577 ??
                                                   |
                  pred again accuracy  =  0.8675599875350577
"""

# take on to test
#log_dir_name_list=log_dir_name_list[1:2]
#Model_List=Model_List[3:4]
