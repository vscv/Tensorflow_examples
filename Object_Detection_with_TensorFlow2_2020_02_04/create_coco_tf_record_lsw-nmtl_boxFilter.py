...
[2021-10-22 0957] Filter bounding box by WH size.

Filter the box_w or box_h less than 300 pixels for train resized image (ex:800x800) without the patching image.
...
# JCK Filter bounding box by WH size 300 pixel.
tf.flags.DEFINE_string('filter_box_size', '300',
                       'The size below will be remove from tfrecord.')
tf.flags.DEFINE_string('num_shards', '10',
                       'The split to how-many tfrecords.')
...
def create_tf_example(image,
  ...
  for object_annotations in annotations_list:
    ...
    if remove_non_person_annotations and category_name != b'person':
      ...
      continue
    # JCK Filter bounding box by WH size 300 pixel.
    if width <= int(FLAGS.filter_box_size) or height <= int(FLAGS.filter_box_size):
      num_annotations_skipped += 1
      continue
...
def main(_):
  ...
  _create_tf_record_from_coco_annotations(
      ...
      val_output_path,
      FLAGS.include_masks,
      num_shards=int(int(FLAGS.num_shards)/2),
