...
#        landmarks.append(y)
        ...
        x = int(float(x)/w * image_w) # to int and range in 512
...
#    landmarks.set_shape([1,1,num_landmarks * 2]) # [32, 102] not [32, 1,1,102], but tensor obj no reshape!
    landmarks = tf.reshape(landmarks, [1, 1, num_landmarks * 2])
...
def CosineDecayCLRWarmUpLSW_3_warmup(epoch):
    ...
    max_lr = 0.005 #1e-3 #1e-2 # currently best for foot pp
...
#    exit()
    ...
    if is_multi_GPU_training == "YES":
        with strategy.scope():
            model = build_model_SeparableConv2D(input_shape, num_landmarks)
    ...
    print(f"image_h:{image_h}, image_w:{image_w}, batch_size:{batch_size}, num_landmarks:{num_landmarks}, num_epochs:{num_epochs}, lr:{lr}, loss:MSE, SavedModel:{SavedModel}")
